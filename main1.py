...
# Uses DriverResults dataclass from driver_results.py
...
from driver_result import DriverResults
    ...
    for f in file_path.glob(f"{name}/*.txt"):
        with open(Path(f'{f.parent}/{f.name}'), 'r') as file:
            ...
            for row in reader:
                ...
                del row['PTS']
                results = {'DATE': f.name[:4], 'FINISH': int(row['POS'])}
                # row['RESULTS'] = [] #{'track': 'dover','date': f.name[:4], 'finish': int(row['POS'])}
